...
module.exports = class TextHandler {
    ...
    async getHandler () {
        ...
        } else if ( this.webhookEvent.message.text === "back" ) {
            let response;
            response = Response.genText("You cannot use back without initiating the communication, please type Hello to began the conversation")
            return response;
        } else {
            response = [
                Response.genText(
