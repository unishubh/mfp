...
module.exports.getUsers = (client_id) =>{
    return new Promise((resolve, reject) =>{
        conn.query("SELECT id, name, fbid, email, phone, gender FROM Users WHERE client_id= ?", [client_id], (err, result) =>{
            if(err){
...
module.exports.getServices = (client_id) =>{
    return new Promise((resolve, reject) =>{
        conn.query("SELECT s.id, u.name, s.details, s.time_received, s.resolved_at, s.is_resolved FROM Users u JOIN Service_requests s ON (u.fbid=s.user_id) WHERE s.client_id = ?",
